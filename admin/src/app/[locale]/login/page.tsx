  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row">
      ...
      <div className="hidden lg:flex lg:w-[45%] bg-deep flex-col justify-between p-10 relative overflow-hidden">
        ...
        <div className="absolute bottom-0 end-0 w-64 h-64 rounded-full bg-secondary/10 blur-[80px] pointer-events-none" />
        ...
        <div className="relative flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-[11px] font-bold text-primary-light mt-0.5">
              {t("brandSubtitle")}
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-12 bg-white relative">
        ...
        <div className="w-full max-w-[360px]">
          <h1 className="text-[26px] font-black tracking-tight text-text-primary">
            {t("headline")}
          </h1>
          ...
          {!inCodeStep && (
            <div className="mt-6 flex rounded-xl border border-border-soft bg-surface-soft p-1 gap-1">
            ...
            <form action={sendAction} className="mt-6 space-y-4">
              ...
              {"error" in (sendState ?? {}) &&
                ...
                "error" in sendState && (
                  <ErrorBanner
                    message={t(
                      `errors.${sendState.error}` as Parameters<typeof t>[0],
                    )}
                  />
                )}
              ...
              <button
                ...
                className="w-full h-10 bg-primary text-white text-sm font-bold rounded-xl hover:bg-[#0D6B63] active:scale-[0.98] transition-all disabled:opacity-60"
              >
          ...
          {mode === "otp" && inCodeStep && (
            <div className="mt-6">
              <div className="flex items-center gap-2 mb-5 p-3 bg-success-light rounded-xl border border-success/20">
                ...
                <button
                  type="button"
                  onClick={resetOtp}
                  className="ms-auto text-[11px] font-semibold text-text-hint hover:text-text-secondary shrink-0 transition-colors"
                >
                  {t("changeNumber")}
                </button>
              </div>
              ...
              <form action={verifyAction} className="space-y-4">
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <input
                    ...
                    autoFocus
                    className="w-full h-14 px-4 rounded-xl border border-border-soft bg-white text-[28px] font-black text-center tracking-[0.5em] text-text-primary placeholder:text-text-hint placeholder:text-[18px] placeholder:tracking-normal focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all"
                  />
                </div>
                ...
                {verifyState?.error && (
                  <ErrorBanner
                    message={t(
                      `errors.${verifyState.error}` as Parameters<typeof t>[0],
                    )}
                  />
                )}
                ...
                <button
                  type="button"
                  onClick={resetOtp}
                  className="w-full h-9 text-sm font-semibold text-text-hint hover:text-text-secondary transition-colors"
                >
                  {t("resendCode")}
                </button>
              </form>
          ...
          {mode === "password" && (
            <form action={passAction} className="mt-6 space-y-4">
              ...
              <PasswordField
                t={t}
                showPass={showPass}
                onToggleShowPass={() => setShowPass((value) => !value)}
              />

              {passState?.error && (
                <ErrorBanner
                  message={t(
                    `errors.${passState.error}` as Parameters<typeof t>[0],
                  )}
                />
              )}
              ...
              <button
                ...
                className="w-full h-10 bg-primary text-white text-sm font-bold rounded-xl hover:bg-[#0D6B63] active:scale-[0.98] transition-all disabled:opacity-60 mt-1"
              >
          ...
          <p className="mt-10 text-center text-xs text-text-hint">
            © 2026 AmanaPOS · Made in Khartoum
          </p>
        </div>
      </div>
