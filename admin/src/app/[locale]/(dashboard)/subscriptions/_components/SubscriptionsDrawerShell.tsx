...
import type { AdminSubscriptionDetail, AdminPlan, AdminOwner } from '@/types/api';
import { cn } from '@/lib/utils';
import {
  CreditCard, Building2, User, Phone, Calendar, Clock,
  Edit2, Ban, Info, Package, Users, Store, ShoppingBag,
  Hash, Search, ChevronRight, ArrowLeft, CheckCircle2, Loader2,
} from 'lucide-react';

export default function SubscriptionsDrawerShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <SubscriptionDrawerContext.Provider value={{ openView, openCreate }}>
      ...
      <Drawer
        ...
        subtitle="Assign a plan to a business"
        width={480}
      >
        <CreateSubscriptionContent onSuccess={handleCreateSuccess} onClose={closeCreate} />
      </Drawer>

      <Drawer open={!!viewId} onClose={closeView} title="Subscription Details" width={560}>
        {viewId && (
...
function CreateSubscriptionContent({
  ...
  const [state, dispatch, isPending] = useActionState<CreateSubscriptionState, FormData>(
  ...
  async function handleOwnerSelect(owner: AdminOwner) {
    ...
    setResolvingId(null);
    if (!result.ok) { setResolveErr(result.error); return; }
    const biz = result.data.businesses.find(b => b.is_active) ?? result.data.businesses[0];
    if (!biz) { setResolveErr(`${owner.full_name} has no registered business yet.`); return; }
    setSelected({ ownerId: owner.id, ownerName: owner.full_name, ownerPhone: owner.phone, businessId: biz.id, businessName: biz.name });
    setStep('form');
  }
  ...
  useEffect(() => {
    ...
    fetchPlansAction().then(result => {
      ...
      else setPlansErr(true);
    });
  }, [step, plans.length]);

  useEffect(() => {
    if (state && 'success' in state && state.success) onSuccess(state.subscription_id);
  }, [state, onSuccess]);
      ...
      <div className="p-5 space-y-4">
        ...
        {resolveErr && (
          <div className="rounded-lg border border-destructive/20 bg-destructive/5 px-3 py-2">
            <p className="text-xs font-semibold text-destructive">{resolveErr}</p>
          </div>
        )}
          ...
          <div className="space-y-1.5">
            {searchResults.map(owner => (
              <button
                ...
                className="w-full flex items-center gap-3 rounded-xl border border-border bg-card hover:bg-muted/40 px-4 py-3 transition-colors text-left disabled:opacity-60"
              >
        ...
        {!searching && searchQuery.trim() && searchResults.length === 0 && !searchErr && (
          <p className="text-sm text-muted-foreground text-center py-4">No owners found for &ldquo;{searchQuery}&rdquo;</p>
        )}
    ...
    <div className="p-5">
      <div className="flex items-center gap-3 rounded-xl border border-success/20 bg-success/5 px-4 py-3 mb-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-[11px] text-muted-foreground truncate">{selected!.businessName} · {selected!.ownerPhone}</p>
        </div>
        <button
          type="button"
          onClick={() => { setStep('search'); setResolveErr(null); }}
      ...
      <form action={dispatch} className="space-y-4">
        ...
        <FormField label="Plan" required>
          {plansErr ? (
            <p className="text-xs text-destructive">Failed to load plans. Refresh and try again.</p>
          ) : plans.length === 0 ? (
                ...
                <optgroup label="— Paid Plans">
                  {paidPlans.map(p => (
                    <option key={p.id} value={p.id}>
                      {p.name} — {parseFloat(p.price).toFixed(0)} {p.currency} / {p.duration_days}d
                    </option>
                  ))}
                </optgroup>
              )}
              {demoPlans.length > 0 && (
                <optgroup label="— Demo Access">
                  {demoPlans.map(p => (
                    <option key={p.id} value={p.id}>
                      {p.name} (Demo) — {p.duration_days}d
                    </option>
                  ))}
                </optgroup>
              )}
        ...
        <FormField label="Payment reference" hint="Invoice number, transaction ID, or any reference.">
          <input name="payment_reference" type="text" placeholder="e.g. INV-2024-001" className={inputCls} />
        </FormField>

        <FormField label="Notes" hint="Internal notes for this subscription.">
          <textarea name="notes" rows={2} placeholder="Optional admin notes…" className={cn(inputCls, 'resize-none')} />
        </FormField>
        ...
        <div className="flex items-start gap-3 rounded-lg border border-warning/20 bg-warning/5 px-4 py-3">
          ...
          <p className="text-xs text-warning/80 leading-relaxed">
            The end date is calculated automatically based on the selected plan&apos;s duration.
            Assigning a paid plan does <strong>not</strong> charge the owner — payment must be collected separately.
          </p>
        </div>
...
function SubscriptionDetailContent({
  ...
  async function load() {
    setLoading(true);
    setErrorMsg(null);
    const result: SubscriptionDetailResult = await fetchSubscriptionDetailAction(subscriptionId);
    if (result.ok) setSub(result.data);
    else           setErrorMsg(result.error);
    setLoading(false);
  }

  useEffect(() => { load(); }, [subscriptionId]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    return (
      <div className="p-5">
        <div className="rounded-xl border border-destructive/20 bg-destructive/5 px-4 py-8 text-center">
          <p className="text-sm font-semibold text-destructive">Failed to load subscription</p>
  ...
  const startDate = new Date(sub.start_date).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' });
  const endDate   = new Date(sub.end_date).toLocaleDateString('en-US',   { month: 'long', day: 'numeric', year: 'numeric' });
  const created   = new Date(sub.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
  const updated   = new Date(sub.updated_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
  ...
  return (
    <div>
        ...
        <div className="px-5 -mt-9 flex items-end gap-4 pb-4 border-b border-border">
          <div className="shrink-0">
            <div className={cn(
              ...
              <CreditCard size={28} className={cn(
                sub.is_expired ? 'text-destructive' : sub.plan_is_free ? 'text-info' : 'text-warning',
              )} />
            </div>
          </div>
      ...
      <div className="px-5 py-3 border-b border-border/60 bg-muted/20 space-y-1">
        <MetaRow icon={<Building2 size={10} />} label="Business" value={sub.business_name} />
        <MetaRow icon={<User     size={10} />} label="Owner"    value={sub.owner_name || '—'} />
        <MetaRow icon={<Phone    size={10} />} label="Phone"    value={sub.owner_phone} mono />
        <MetaRow icon={<Calendar size={10} />} label="Period"   value={`${startDate} → ${endDate}`} />
        <MetaRow icon={<Clock    size={10} />} label="Created"  value={created} />
        <MetaRow icon={<Clock    size={10} />} label="Updated"  value={updated} />
        {sub.payment_reference && (
          <MetaRow icon={<Hash size={10} />} label="Ref" value={sub.payment_reference} mono />
        )}
      </div>
      ...
      <div className="px-5 py-4 border-b border-border">
        <div className="grid grid-cols-3 gap-2">
          <StatBox
            ...
            color={sub.is_expired ? 'text-destructive' : sub.days_remaining <= 7 ? 'text-warning' : 'text-success'}
            bg={sub.is_expired ? 'bg-destructive/10' : sub.days_remaining <= 7 ? 'bg-warning/10' : 'bg-success/10'}
          />
          <StatBox
            label="Max Shops"
            value={`${sub.max_shops}`}
      ...
      <div className="px-5 py-4 border-b border-border">
        ...
        <div className="grid grid-cols-3 gap-3">
          <PlanLimitItem icon={<Store size={13} />}       label="Shops"    value={sub.max_shops} />
          <PlanLimitItem icon={<Package size={13} />}     label="Products" value={sub.max_products} />
          <PlanLimitItem icon={<Users size={13} />}       label="Users"    value={sub.max_users} />
        </div>
        <div className="mt-3 flex items-center gap-2 text-xs text-muted-foreground">
          <ShoppingBag size={11} />
          <span>Duration: <strong className="text-foreground">{sub.plan_duration} days</strong></span>
          {!sub.plan_is_free && (
            <>
      ...
      <div className="px-5 py-4 space-y-2">
        ...
        {!sub.is_active && (
          <div className="flex items-center gap-3 rounded-xl border border-border bg-muted/30 px-4 py-3">
            ...
            <p className="text-xs text-muted-foreground">
              This subscription has been deactivated and cannot be reactivated from here.
            </p>
          </div>
        )}
      </div>
...
function EditSubscriptionInline({
  ...
  const [state, dispatch, isPending] = useActionState<UpdateSubscriptionState, FormData>(action, null);
  ...
  return (
    <>
      ...
      >
        ...
        <div>
          <p className="text-[13px] font-semibold text-foreground">Edit Notes & Reference</p>
          <p className="text-[11px] text-muted-foreground mt-0.5">Update payment reference or admin notes</p>
        </div>
      </button>
...
function DeactivateSubscriptionInline({
  ...
  async function handleConfirm() {
    ...
    if (!result || 'error' in result) { setError(result?.error ?? 'Unknown error'); return; }
  ...
  return (
    <>
      ...
      <button
        ...
        className="w-full flex items-center gap-3 rounded-xl border border-destructive/20 bg-destructive/5 hover:bg-destructive/10 px-4 py-3 transition-colors text-left"
      >
        ...
        <div>
          <p className="text-[13px] font-semibold text-destructive">Deactivate Subscription</p>
          <p className="text-[11px] text-destructive/60 mt-0.5">Immediately disables access for this business</p>
        </div>
      </button>
      ...
      <ConfirmDialog
        ...
        description="This will immediately deactivate the subscription. The business will lose access to plan features. This cannot be undone from the dashboard."
